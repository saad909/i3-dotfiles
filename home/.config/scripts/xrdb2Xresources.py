...
class XrdbEntry(object):
    def __init__(self, define: str, key: str, *value: str):
        super().__init__()
        self.define = define
        self.key = key.lower()
        self.value = value
...
def main(xrdb_path, output_path):
    file = xrdb_path
    # for f in glob(join(xrdb_path, '*.xrdb')):
    xrdb_in = read_xrdb(file)
    base_name = splitext(basename(file))[0]
    x_resources_out = output_path and join(output_path, base_name + ".conf") or sys.stdout
